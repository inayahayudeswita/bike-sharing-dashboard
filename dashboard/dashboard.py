...
try:
    ...
    model_day = joblib.load(os.path.join(BASE_DIR, "randomforest_day_model.pkl"))
    model_hour = joblib.load(os.path.join(BASE_DIR, "randomforest_hour_model.pkl"))
    day_df, hour_df = load_data()
    st.sidebar.success("✅ Data dan model berhasil dimuat!")
    data_loaded = True
except Exception as e:
    st.error(f"Error saat memuat data atau model: {e}")
    st.info("Pastikan file model berada di folder yang sama dengan dashboard.py")
    data_loaded = False
...
if data_loaded:
    ...
    with tab2:
        ...
        if data_view in ["Harian (day)", "Keduanya"]:
            ...
            with col1:
                st.pyplot(create_season_pie(day_df, "Distribusi Penyewaan Sepeda Berdasarkan Musim (Day)", "pastel", 'cnt_x'))
            with col2:
        ...
        if data_view in ["Per Jam (hour)", "Keduanya"]:
            ...
            col1, col2 = st.columns(2)
            with col1:
                st.pyplot(create_season_pie(hour_df, "Distribusi Penyewaan Sepeda Berdasarkan Musim (Hour)", "Reds", 'cnt_y'))
            with col2:
    ...
    with tab3:
        ...
        if mode == "Harian (day)":
            ...
            with col2:
                holiday = st.selectbox("Hari Libur (0=Tidak, 1=Ya)", [0, 1])
                weekday = st.slider("Hari ke-", 0, 6, 3)
                workingday = st.selectbox("Hari Kerja (0=Tidak, 1=Ya)", [0, 1])
            with col3:
                weathersit = st.selectbox("Cuaca", [1, 2, 3, 4], format_func=lambda x: {1: 'Cerah', 2: 'Berawan', 3: 'Hujan', 4: 'Badai'}[x])
                temp = st.slider("Suhu (temp)", 0.0, 1.0, 0.5)
                atemp = st.slider("Suhu Terasa (atemp)", 0.0, 1.0, 0.5)
                hum = st.slider("Kelembaban (hum)", 0.0, 1.0, 0.5)
                windspeed = st.slider("Kecepatan Angin (windspeed)", 0.0, 1.0, 0.2)
            ...
            with col1:
                ...
                yr = st.selectbox("Tahun (0=2011, 1=2012)", [0, 1], key="yr_hour")
                mnth = st.slider("Bulan", 1, 12, 6, key="mnth_hour")
            with col2:
                hr = st.slider("Jam (0-23)", 0, 23, 12, key="hr_hour")
                holiday = st.selectbox("Hari Libur (0=Tidak, 1=Ya)", [0, 1], key="hol_hour")
                weekday = st.slider("Hari ke-", 0, 6, 3, key="weekday_hour")
                workingday = st.selectbox("Hari Kerja (0=Tidak, 1=Ya)", [0, 1], key="work_hour")
            with col3:
                weathersit = st.selectbox("Cuaca", [1, 2, 3, 4], key="weather_hour",
                                          format_func=lambda x: {1: 'Cerah', 2: 'Berawan', 3: 'Hujan', 4: 'Badai'}[x])
                temp = st.slider("Suhu (temp)", 0.0, 1.0, 0.5, key="temp_hour")
                atemp = st.slider("Suhu Terasa (atemp)", 0.0, 1.0, 0.5, key="atemp_hour")
                hum = st.slider("Kelembaban (hum)", 0.0, 1.0, 0.5, key="hum_hour")
                windspeed = st.slider("Kecepatan Angin (windspeed)", 0.0, 1.0, 0.2, key="wind_hour")
